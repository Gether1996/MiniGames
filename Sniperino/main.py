...
while True:
    ...
    while stage_of_game in [1, 2, 3, 4]:
        ...
        back_to_menu_text = FONT.render("MENU", True, WHITE)
        shark_key = FONT.render("Y", True, WHITE)
        push_key = FONT.render("X", True, WHITE)
        score_surface = FONT.render(f"Score: {score}", True, WHITE)
        current_stage_surface = FONT_BIGGER.render(f"Stage: {stage_of_game}", True, WHITE)
        velocity = 0.2
